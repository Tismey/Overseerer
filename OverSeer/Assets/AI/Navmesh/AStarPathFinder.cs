...
public class AStarPathFinder
{
    ...
    public static List<Vector3> FindPathWithCover(Vector3 worldStart, Vector3 worldEnd, int direction)
    {
        // direction ∈ [0..7] (même système que tes offsets)
        const float COVER_BONUS = 0.8f;
        // plus bas = plus agressif pour chercher le couvert
        // 0.65f = bon compromis, mais tu peux ajuster
        ...
        if (startGrid.heights == null || targetGrid.heights == null)
            return null;
        ...
        while (openList.Count > 0)
        {
            ...
            // End reached in 3D (including height)
            if (current.x == tx && current.y == ty && current.h == th)
                return SimplifyPath(ReconstructPath(current));
            ...
            for (int d = 0; d < 8; d++)
            {
                ...
                if (!node.connections[current.h][d])
                    continue;
                ...
                float newG = current.g + baseCost;

                // ---------------------------------------------------------
                // COVER BONUS : Réduit le coût des cases où on est couvert
                // ---------------------------------------------------------
                if (neigh.cover != null &&
                    direction >= 0 && direction < 8 &&
                    neigh.cover.Length > nh &&
                    neigh.cover[nh][direction])
                {
                    newG *= COVER_BONUS; // ✔ réduit le coût donc préféré
                }
    ...
    public static List<Vector3> SimplifyPath(List<Vector3> path, bool ignoreY = true)
    {
